...
Three classes of outcomes are tracked separately so the report tells the
...
- `created`           — the row was successfully accepted

The report becomes the system-under-test for the Day 3 test suite.
"""
...
def import_csv(
    ...
    with csv_path.open() as fh:
        ...
        for row_no, row in enumerate(reader, start=2):  # row 1 is header
            try:
                payload = ProductCreate.model_validate(row)
            except ValidationError as exc:
                logger.warning("row %d failed validation", row_no)
            ...
            try:
                ...
                logger.warning("row %d API error: %s", row_no, exc)
                api_errors.append(
                    {"row": row_no, "input": row, "status": exc.status_code, "detail": exc.detail}
                )
                continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        logger.error("server not reachable at %s: %s", args.base_url, exc)
        return 2
